...
" class='addRepetitionButton btn new'style='display:block;'>+</a>
	...
	<div id='eventWillStop-wrapper'>
		...
		<select name="editEventForm[will_stop]" id="editEventForm-willStop" data-validators='required'>
			<option value="0" <?php if ($_smarty_tpl->tpl_vars['eventModel']->value['will_stop']==='0'){?>selected='selected'<?php }?>>Never</option>
			<option value="1" <?php if ($_smarty_tpl->tpl_vars['eventModel']->value['will_stop']==='1'){?>selected='selected'<?php }?>>On date</option>
		</select>
	</div>
