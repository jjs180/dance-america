...
$j('.addRepetitionButton').click(function() {
	...
	var index = parseInt($j(this).attr('id').substring(20, 22))+1;
...
$j('#addCost-button').click(function(){
	if ($j(this).text() == 'Click to put in cost details') {
		...
		$j('.costDetails-container').removeClass('hidden');
	} else {
		addCost();
	}
});
...
function addCost() {
	...
	$j('.costDetails-rhsWrapper').append("<div><input type='text' name='addEventForm[costs]["+indexForCost+"][person_type]' class='person-type' placeholder='Type (eg members, non-members, etc)' /> pay $ <input type='text' name='addEventForm[costs]["+indexForCost+"][amount]' placeholder='Amount (USD)' data-validators='validate-numeric' /></div>");
}
